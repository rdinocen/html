...
(function($, window){
	...
	dragableColumns = (function(){
		...
		function dragColumns (table, options) {
			...
			$rows = $table.find(_this.options.movedContainerSelector);
		...
		dragColumns.prototype = {
			...
			handleDrop: function (e) {
				...
				var cb = _this.options.onReorder;
                if (cb) { e.currentTarget = $table[0]; cb(e); }
				return;
			},
			...
			moveColumns: function (fromIndex, toIndex) {
				var fromCol = fromIndex + 1;
				var toCol = toIndex + 1;
				console.log('moving rows from column ' + fromCol + ' to column ' + toCol);
				/*
				var wrapper = $table.parent().closest('div')
				console.log(wrapper)
				console.log($table);
				//var rows = wrapper.find(_this.options.movedContainerSelector);
